...
int gNum_procs;
int g_pcdn_thread_num;
int g_scdn_thread_num;
int g_bundle_size;
...
void do_infer(FILE *input, FILE *output, struct Model* model_)
{
	int correct = 0;
	int total = 0;
	...
	while(readline(input) != NULL)
	{
		...
		while(1)
		{
			...
			{
				max_nr_attr *= 2;
				x = (struct FeatureNode *) realloc(x,max_nr_attr*sizeof(struct FeatureNode));
			}
		...
		if(flag_predict_probability)
		{
			...
			for(j=0;j<model_->num_class;j++)
				fprintf(output," %g",prob_estimates[j]);
			fprintf(output,"\n");
		}
		else
		{
			predict_label = infer(model_,x);
			fprintf(output,"%d\n",predict_label);
		}
...
{
	printf(
	//"Usage: infer [options] test_file model_file output_file\n"
	//"options:\n"
...
int main(int argc, char **argv)
{
	...
	if((gModel=load_model(argv[i+1]))==0)
	{
	...
	x = (struct FeatureNode *) malloc(max_nr_attr*sizeof(struct FeatureNode));
	do_infer(input, output, gModel);
